...
class TestUpdateUser:
    ...
    def test_update_user_data(self, get_token):
        user_before = requests.get(Url.UPDATE_USER, headers={'Authorization': get_token} )
        payload = {
            "email": "update_",
            "name": "update_name"
        }

        response = requests.patch(Url.UPDATE_USER, headers={'Authorization': get_token}, data=payload)
        user_after = requests.get(Url.UPDATE_USER, headers={'Authorization': get_token})
